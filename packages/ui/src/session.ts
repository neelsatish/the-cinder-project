...
export function createSessionOperationQueue() {
  let tail = Promise.resolve();
  return function enqueue<T>(operation: () => Promise<T> | T) {
...
const runSessionOperation = createSessionOperationQueue();
 ...
 */
export function loadSessionValue(keys: readonly string[]) {
...
export function saveSessionValue(
...
export function clearSessionValue(keys: readonly string[]) {
  return runSessionOperation(async () => {
    if (isTauriRuntime()) {
      try {
